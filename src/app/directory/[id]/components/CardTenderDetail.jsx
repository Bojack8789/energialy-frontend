    ...
    useEffect(() => {
      const init = async () => {
        try {
          ...
          // tw-elements v2.0.0 no requiere inicialización explícita
          // El Ripple se inicializa automáticamente con data-te-ripple-init
        } catch (error) {
  ...
  return (
    <>
      {tenders?.map((tender, index) => (
        <div key={index} className="rounded-lg bg-white p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 md:min-w-[1000px] mb-4 flex justify-between">
