...
export default function EditTenderForm({ params }) {
  ...
  const { data: tender, isLoading: tenderLoading } =
    useGetTenderByIdQuery(tenderId);
  ...
  // Selects controlados (para reflejar valores precargados)
  ...
  useEffect(() => {
    if (tender && !dataLoaded) {
      ...
      setTenderData({
        ...
        showBudget: tender.showBudget ?? true,
        public: tender.public ?? true,
        ...
        locationId: tender.location?.id || "",
        subcategories: tender.subcategories
          ? tender.subcategories.map((s) => s.id)
          : [],
      ...
      setIsShow(tender.showBudget ?? true);
      setIsPrivateCheqed(tender.public === false);
      setCustomFields(tender.customFields || []);
      setServicePrices(
        (tender.servicePrices || []).map((s) => ({
      ...
      const firstCategory = tender.categories?.[0];
      if (firstCategory) {
        ...
        const subcategories =
          displayCategories?.find((cat) => cat.id === firstCategory.id)
            ?.subcategories || [];
        setSubCatSelected(
      ...
      const firstSubcategory = tender.subcategories?.[0];
      if (firstSubcategory) {
        setSelectedSubcategoryOption({
          value: firstSubcategory.id,
  ...
  }, [tender, dataLoaded, displayCategories, router, userData]);
  ...
  const handleChangeCategories = (e) => {
    ...
    setTenderData({ ...tenderData, subcategories: [] });
  };
  const handleSubcategorieChange = (e) => {
    setSelectedSubcategoryOption(e);
    const arr = [];
    arr.push(e.value);
  ...
  const handleAddCustomField = () => {
    ...
    const updated = [...customFields, fieldToAdd];
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hasErrors) {
      try {
        ...
        await axios.put(`${urlProduction}/tenders/${tenderId}`, payload, {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        });
        displaySuccessMessage("Licitación actualizada con éxito");
        ...
        const msg =
          error?.response?.data?.error || "Error al actualizar la licitación";
        displayFailedMessage(msg);
      }
    }
  };

  if (tenderLoading || !dataLoaded) {
    return (
        ...
        <Card className="p-4">
          ...
          <div className="border-b-1">
            ...
            <Typography variant="small" className="mb-4">
              Modifica los datos de la licitación publicada.
            </Typography>
          </div>
          {/*First Step Data*/}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <select
                    ...
                    onChange={handleInputsChanges}
                    value={tenderData.projectDuration}
                  >
                    <option value="">DURACIÓN DE LA LICITACIÓN</option>
          ...
          <div className="flex gap-3 mt-4">
            <button
              type="button"
              onClick={() => router.back()}
              className="border border-gray-300 text-gray-700 font-semibold rounded-md px-8 py-3 hover:bg-gray-50 transition-colors duration-200"
            >
              Cancelar
            </button>
            <button
              className="flex-1 sm:flex-none bg-[#191654] hover:bg-[#252075] text-white font-semibold rounded-md px-8 py-3 transition-colors duration-200"
