...
function MenuItem({ menuItem, index, isOpen, user, isBankAccountOpen, onExpand }) {
  ...
  return (
    <>
      {user ? (
        <li
          key={index}
          className={`text-gray-800 text-sm  cursor-pointer flex items-center p-2 hover:bg-slate-100 rounded-md ${
      ...
      {menuItem.submenu && isOpen && subMenuOpen && (
        <ul>
          {menuItem.submenuItems.map((item, index) => (
            <li
              key={index}
              className={`text-gray-600 text-xs flex items-center gap-x-4 cursor-pointer p-1 px-3 hover:bg-slate-100 rounded-md`}
