...
export const UserList = () => {
  ...
  const tableColumns = [
    ...
    { header: 'Plan', accessor: 'subscription', type: 'status' },
  ...
  return (
    <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-title-md2 font-semibold text-black dark:text-white">Gestión de Usuarios</h2>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-sm border border-stroke bg-white px-7.5 py-6 shadow-default">
          <h4 className="text-title-md font-bold text-black">{users.length}</h4>
          <span className="text-sm font-medium">Total Usuarios</span>
        </div>
        <div className="rounded-sm border border-stroke bg-white px-7.5 py-6 shadow-default">
          <h4 className="text-title-md font-bold text-black">{users.filter(u => u.isActive).length}</h4>
